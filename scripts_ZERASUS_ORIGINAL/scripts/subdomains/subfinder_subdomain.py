...
container_name = f"{target}-{x}-subfinder"
saida = f"subfinder-{x}.txt"
...
def executa():
    run = subprocess.check_output(
        f'docker run --name {container_name} --rm -v /data/{target}/tmp:/data kali-tools:1.0 subfinder -d {dominio} -oJ -silent >>  /data/{target}/tmp/{saida} || true',
        shell=True)
...
def parse():
    with open(f'/data/{target}/tmp/{saida}') as file:  # abrindo arquivo
        for linha in file:
            json_line = linha.rstrip()
            json_data = json.loads(json_line)
            dicionario['server.domain'] = dominio
            dicionario['timestamp'] = hora
            dicionario['server.address'] = json_data['host']

            try:
                dicionario['server.ip'] = socket.gethostbyname(json_data['host'])
            except:
